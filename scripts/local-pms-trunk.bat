...
echo ######################################
echo #                                    
echo #			PMS Build Module          
echo #                                    
echo #                                    
echo ######################################


del "C:\logs\pmsbuild.log" /Q
SET LOG_LOCATION=C:\logs\pmsbuild.log
SET PROD_BASE=C:\hcentive-code\local\private-exchange
SET PMS_BASE=C:\hcentive-code\local\pms-trunk
...
cd %PMS_BASE%\build\pms-ws-build
echo [PROGRESSING] :: pms-ws-build :: > CON
call mvn clean install -f pom.xml >> "%LOG_LOCATION%" 2>&1
if "%ERRORLEVEL%" == "0" (echo [SUCCESSFUL]  :: pms-ws-build ::) else (echo [FAILED] :: pms-ws-build ::)
echo.
